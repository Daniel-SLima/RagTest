...
from app.rag.loaders import discover_source_files, load_docx, load_pdf, load_source_documents
...
def test_load_pdf_preserves_page_number_and_metadata(tmp_path: Path) -> None:
...
def test_load_source_documents_keeps_processing_after_bad_file(tmp_path: Path) -> None:
    ...
    report = load_source_documents(tmp_path)
