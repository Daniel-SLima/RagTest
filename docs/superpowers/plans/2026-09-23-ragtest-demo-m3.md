...
**Goal:** Conectar a aba `Chat` da Demo Técnica ao replay live controlado do M1, apresentar apenas os DTOs sanitizados recebidos e reutilizar a mesma execução na aba `Como funciona`, sem ampliar o backend, o contrato ou o Laboratório do M4.
...
- A validação de `baseUrl` aceita somente `http` para `localhost`, `127.0.0.0/8`, `::1` ou rede privada local RFC1918, e aceita `https` explicitamente. Rejeita credenciais, query string, hash, protocolo diferente de HTTP(S), host ausente, URL malformada e HTTP público não local. Não implementa autenticação completa.
...
- A rota M1 chama `answer_with_rag(..., auto_decompose=False)`: o M3 deve declarar visualmente o caso como single-query. Multi-query, decomposição, retry count, ranking pré/pós, contexto final enviado ao LLM e dimensão do embedding ficam como `Não disponível`/indisponível; não podem ser inferidos de `sources`, scores, timings ou texto da resposta.
...
- **Allowlist de origem:** URLs locais confiáveis e HTTPS explícito passam; credenciais, query/hash, `file:`, host público em HTTP e URL inválida falham antes de qualquer request. Cobrir em `demo-api.test.ts`.
- **Request única:** `getRuntime` acontece uma vez por montagem; `run` só acontece no submit/retry explícito; render, construção, exemplos e troca de aba não fazem chamada. Cobrir em `app.test.tsx` e `use-demo-run.test.ts`.
...
- **Granularidade honesta:** single-query é explícito porque a rota usa `auto_decompose=False`; decomposition, multi-query, retry count, pré/pós-reranking, contexto final e dimensão de embedding aparecem como indisponíveis. Cobrir no modelo de pipeline.
...
- Create `frontend/src/demo/types/run-state.ts`: `DemoRunStatus = "idle" | "loading" | "success" | "error"`, `DemoRunDiagnostics`, `DemoRunState` e tipos dos callbacks. `diagnostics` é projeção somente leitura de `response`/`runtime`, não uma segunda fonte mutável.
- Create `frontend/src/demo/state/demo-run-state.ts`: reducer/funções puras para `idle`, `loading`, `success` e `error`; `toDemoDiagnostics(response, runtime)` copia apenas campos dos DTOs sanitizados.
- Create `frontend/src/demo/state/use-demo-run.ts`: `useDemoRun({ api, runtime })` com `submit(question)` e `retry()`; valida localmente, chama `api.run` uma vez por ação e conserva a pergunta após erro.
- Create `frontend/src/demo/state/use-demo-run.test.ts`: RED/GREEN de loading, sucesso, erro, retry, campos null, fontes vazias, sem raw body e sem chamada duplicada.
...
- Create `frontend/src/demo/state/use-demo-runtime.ts`: `getRuntime` uma vez por montagem, com `useRef`/dependências estáveis e estado sanitizado de loading/sucesso/erro.
...
- `validateDemoBaseUrl(value: string): string` aceita apenas a política descrita em Global Constraints e retorna a URL sem barra final, ou lança `{ code: "invalid_demo_base_url", status: null, message: "URL do serviço de demonstração inválida." }`.
...
- Create: `frontend/src/demo/state/use-demo-run.ts`
- Create: `frontend/src/demo/state/use-demo-runtime.ts`
- Create: `frontend/src/demo/state/use-demo-run.test.ts`
- Create: `frontend/src/demo/state/use-demo-runtime.test.ts`

**Interfaces:**

- `DemoRunState` expõe `question: string | null`, `response: DemoRunResponse | null`, `diagnostics: DemoRunDiagnostics | null`, `runtime: DemoRuntime | null`, `status`, `error: DemoApiError | null`. `DemoRunDiagnostics` contém somente `answer`, `model`, `grounded`, `citation_ids`, `sources`, `timings` e `runtime`; é calculado de forma pura de `response`/`runtime` e não recebe payload externo.
- `useDemoRuntime(api): { runtime: DemoRuntime | null; status: "loading" | "success" | "error"; error: DemoApiError | null }` chama `api.getRuntime()` uma vez por montagem. Uma falha deixa runtime `null` e mensagem sanitizada; não repete em troca de aba.
- `useDemoRun({ api, runtime }): { state: DemoRunState; submit(question: string): Promise<void>; retry(): Promise<void> }` faz uma chamada `api.run` somente por submit/retry explícito, mantém `question` após erro e não implementa retry automático.

- [ ] **Step 1: Write the failing tests.** Crie reducer/hook tests com `DemoApi` mockado: estado idle; pergunta inválida não chama API; submit emite loading e depois sucesso com runtime e diagnóstico; erro HTTP/rede preserva pergunta sem raw body; retry explícito produz exatamente uma segunda chamada; scores/timings nullable permanecem null; `getRuntime` é chamado uma vez no mount mesmo após rerender; unmount/aba não dispara run.
- [ ] **Step 2: Run focused tests to verify RED.** Executar `cd frontend; npm test -- --runTestsByPath src/demo/state/use-demo-run.test.ts src/demo/state/use-demo-runtime.test.ts`; esperar falha por hooks/reducer inexistentes.
- [ ] **Step 3: Write the minimal implementation.** Implementar reducer e hooks com `useMemo` para cliente estável, `useRef` para impedir chamadas duplicadas da mesma montagem e `useEffect` somente para runtime. `submit` deve trimar a pergunta, limpar resposta/erro anterior, marcar loading, chamar `api.run({ query })` e preencher estado apenas com o DTO validado. `retry` deve reutilizar `state.question` e retornar cedo durante loading ou sem pergunta.
...
- `DemoApp` executa `useDemoRuntime` uma vez, `useDemoRun` com o runtime atual, mantém `activeTab`, `pipelineMode` e `presentationIndex`, e passa callbacks puros para Chat/Como funciona.
...
- [ ] **Step 3: Write the minimal implementation.** Criar cliente com dependências estáveis, capturar erro de base URL em estado de configuração sem renderizar corpo de exceção e fazer `getRuntime` em `useEffect` controlado. `handleSubmit` chama somente `submit`; `handleRetry` chama somente `retry`; `handleViewPipeline` muda aba/modo/índice. A troca de aba apenas seleciona tela; nenhum efeito depende de `activeTab` para buscar dados.
