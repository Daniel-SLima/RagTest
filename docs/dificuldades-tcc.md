...
Correção: nenhuma correção de comportamento foi aplicada ainda. Como próximo diagnóstico mínimo, executar a mesma pergunta RAG com override temporário de `LLM_MAX_OUTPUT_TOKENS=512`, mantendo modelo, contexto, retrieval, corpus e prompt iguais. O objetivo é testar isoladamente se o orçamento de saída elevado é o principal fator da latência antes de instrumentar ou alterar o provider.
