...
Diagnóstico: a integração de transporte e autenticação Groq está funcionando e a latência é muito inferior à do Qwen3 8B local. A primeira falha pode ter sido influenciada pelo teto de 512 tokens, pois terminou por `length`, mas a segunda tentativa terminou normalmente e ainda assim não produziu citações, portanto o problema não pode ser atribuído apenas ao limite de saída. Um teste direto posterior retornou exatamente `Direito teste [1].`, eliminando a hipótese de incapacidade do modelo/provider de produzir o formato literal. A investigação agora se concentra no prompt RAG completo, no contexto documental e na distribuição do orçamento entre reasoning e resposta final.

Correção: a chamada direta ao mesmo `GroqProvider`, sem retrieval e com limite de 256 tokens, retornou exatamente `Direito teste [1].`. Isso valida que o provider e o GPT-OSS 120B conseguem produzir o formato literal exigido. Nenhuma correção de comportamento foi aplicada ainda; o próximo diagnóstico é capturar uma geração RAG bruta, antes do gate, usando apenas `direitos_saude`, para identificar se o prompt completo está desviando o formato ou consumindo o orçamento em outro tipo de conteúdo. Não afrouxar o gate antes dessa observação.
