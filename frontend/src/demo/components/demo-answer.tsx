...
export function DemoAnswer({ response }: { response: DemoRunResponse }) {
  ...
  return <AnswerPresentation model={{ answer: response.answer, grounded: response.grounded, sourcesTitle: response.grounded ? "Fontes consultadas" : "Fontes recuperadas para consulta", showCitationId: response.grounded, sources: sources.map((source) => ({ key: `${source.public_id}-${source.order}`, citationLabel: `[${source.order}]`, document: source.document, page: source.page, excerpt: source.excerpt, scores: { Dense: source.scores.dense_score, Sparse: source.scores.sparse_score, Ranking: source.scores.rank_score, Fusão: source.scores.fusion_score } })) }} />
}
