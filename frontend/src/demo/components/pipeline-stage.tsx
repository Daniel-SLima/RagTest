...
export function PipelineStage({ title, simpleExplanation, technicalDetails, state, expanded, onToggle }: PipelineStageProps) {
  ...
  return <Pressable accessibilityHint="Expande ou recolhe os detalhes técnicos" accessibilityLabel={title} accessibilityRole="button" accessibilityState={{ expanded }} onPress={onToggle} style={({ pressed }) => [styles.stage, { borderColor: theme.border, backgroundColor: theme.surface }, pressed && styles.pressed]}><View style={styles.heading}><Text style={[styles.title, { color: theme.text }]}>{title}</Text><Text style={[styles.state, { color: theme.warning }]}>{state}</Text></View><Text style={{ color: theme.textMuted }}>{simpleExplanation}</Text>{expanded ? <Text style={[styles.details, { color: theme.text }]}>{technicalDetails}</Text> : null}</Pressable>
}
