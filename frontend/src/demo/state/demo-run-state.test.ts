...
describe("demoRunReducer", () => {
  ...
  it("preserves the completed run when runtime later fails", () => {
    ...
    expect(failed.runtimeError).toBe(apiError)
  })
  it("does not accept raw response or errors in state", () => {
    const errorState = demoRunReducer(initialDemoRunState, { type: "run-error", error: apiError })
    expect(errorState).toMatchObject({ error: apiError })
