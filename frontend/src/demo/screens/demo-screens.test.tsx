...
describe("M2 screens", () => {
  ...
  it("shows all four neutral screen areas without fabricated results", async () => {
    ...
    await renderDemo(<LaboratoryScreen />)
    expect(screen.getByText("Compare o retrieval documental real. Esta superfície não gera resposta e não chama um provedor de LLM.")).toBeTruthy()
    expect(screen.getByText("Multi-query / decomposição")).toBeTruthy()
    expect(screen.getByText(/Indisponível nesta versão/)).toBeTruthy()
    expect(screen.getByRole("button", { name: "Executar retrieval" }).props.accessibilityState).toEqual({ disabled: true })
