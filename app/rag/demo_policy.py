...
_DOTENV_RE = re.compile(r"(?i)(?:\.env|authorization)\s*[:=]?\s*\S*")
...
_UNSAFE_RUNTIME_RE = re.compile(r"(?i)(?:https?://|[a-z]:\\|\.env|api[_ -]?key|token|secret)")
...
def sanitize_runtime_label(value: str, *, limit: int = 120) -> str:
    ...
    if _UNSAFE_RUNTIME_RE.search(text):
        return "configured"
    return text[:limit]
...
class DemoSourcePolicy:
    ...
    def __init__(self, prefixes: str) -> None:
        self.prefixes = tuple(
            prefix.strip().replace("\\", "/").lower().lstrip("/")
            for prefix in prefixes.split(",")
            if prefix.strip()
        )

    def allows(self, source: str) -> bool:
        normalized = source.strip().replace("\\", "/").lower().lstrip("/")
        if not normalized or "chatscm" in normalized:
            return False
        parts = set(PurePosixPath(normalized).parts)
        if any(part == "private" or part.startswith("private_") for part in parts):
            return False
        return bool(self.prefixes) and any(
            normalized.startswith(prefix) for prefix in self.prefixes
        )
